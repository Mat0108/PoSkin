...
const DiagnosticStart = (props)=>{
    ...
    const Element = useMemo(() =>{        
        return <div className="flex flex-col h-full  sm:h-full ">
            
            <div className="w-full h-full flex flex-row">
                <div className="invisible sm:visible w-0 sm:w-1/3 h-full relative">
                    <img src={"/images/Diagnostic/diagnostic5.png"} alt={"diagnostic"} className="w-full h-full"/>
                </div>
                <div className="w-full sm:w-2/3 h-full bg-[#EEE8E4] relative">
                    ...
                    <div className="ml-[2%] sm:ml-[5%] flex flex-col ">
                        <div className={`flex flex-col sm:flex-row w-[90%] mt-[10px] sm:mt-[40px]  text-[16px] sm:text-[28px] text-[#264C4D] text-left h-fit mb-[10px] gap-2 ${i===DiagnosticData.length-1 ? "font-mt-demi":""}`}>
                            <div className="font-mt-bold">{(i === DiagnosticData.length -1 && typeof cookies.user === "object" ) ? `${dictionnaire.DiagnosticStart.trace}` : `${DiagnosticData[i].section} ${DiagnosticData[i].section ? ":": ""} `}</div>
                            <div>{(i === DiagnosticData.length-1 && typeof cookies.user === "object" ) ? "": DiagnosticData[i].titre} </div>
                        </div>
                        <div><h2 className={`w-[90%] mb-[10px] text-[10px] sm:text-[20px] text-[#264C4D] text-left h-fit mb-[10px] ${i===DiagnosticData.length-1 ? "font-mt-demi":""}`}>{(i === DiagnosticData.length-1 && typeof cookies.user === "object" ) ? "": `${DiagnosticData[i].question }`} </h2></div>
                        ...
                        <div>
                            {i === DiagnosticData.length-1 ?<div className={`grid grid-cols-1 grid-rows-5 mb-[20px]`}>
                                ...
                                <input
                                    ...
                                    required
                                />
                                <div className="flex flex-col w-1/3 center ml-[10px] "> <div className="w-fit bg-[#264C4D] rounded-full text-[12px] sm:text-[24px] px-16 py-2 text-white hover:cursor-pointer" onClick={()=>{Envoyer()}}>{dictionnaire.send}</div> </div>
